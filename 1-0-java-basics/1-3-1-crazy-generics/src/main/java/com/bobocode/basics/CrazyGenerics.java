...
public class CrazyGenerics{
    ...
    static class CollectionUtil {
         ...
         *
         * @param entities            provided collection of entities
         * @param validationPredicate criteria for validation, if predicate can validate objects
         *                            -> it is also valid predicate for current case
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            ...
            while (iterator.hasNext()) {
                T buff = iterator.next();
                if (comparator.compare(buff, max) > 0)
                    max = buff;
            }
